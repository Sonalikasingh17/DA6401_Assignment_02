...
def test():
    ...
    test_loader = prepare_test_dataset(augment_data=config.augment_data, image_size=256)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = CNNClassifier(num_filters=config.num_filters, filter_multiplier=config.filter_multiplier,
                          dropout=config.dropout, batch_norm=config.batch_norm, dense_size=config.dense_size).to(device)
    ...
    # Plot results
    plot_test_results(all_images, all_preds, all_labels)
    plot_filters(model, all_images[11])
